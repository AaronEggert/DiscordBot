...
function cmd_test(msg, args) {
	...
	collector.on('end', async collected => {
		...
		msg.reply(
			'Nenne bitte jetzt den Channel in der die Nachricht gesendet werden soll: '
		);
		const filter = newMessage => {
			return newMessage.author.id === msg.author.id;
		};
		...
		collector.on('end', async collected => {
			...
			msg.reply(
				'Nenne bitte jetzt den Titelder Nachricht die gesendet werden soll: '
			);
			const filter = newMessage => {
				return newMessage.author.id === msg.author.id;
			};
...
function cmd_set(msg, args, author) {
	...
	fs.writeFile(
		'./curency.json',
		JSON.stringify(client.curency, null, 4),
		err => {
			if (err) throw err;
		...
		if (msg.author.id == config.owner) {
			...
			msg.channel.send({
				embed: {
					color: 0xe74c3c,
					description:
						':no_entry_sign:  Du hast keine Befugnis diesen Befehl zu nutzen!  :no_entry_sign:'
				}
			});
			return;
		}
...
function cmd_clear(msg, args, author) {
	...
	msg.channel.bulkDelete(amount, true).then(_messages => {
		msg.channel
			.send(`Bot cleared \`${_messages.size}\` messages :broom:`)
			.then(sent => {
				setTimeout(function() {
					sent.delete();
				}, 2500);
